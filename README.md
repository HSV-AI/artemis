...
- PI may use the explicitly registered `web_fetch` tool to read a user-provided HTTP or HTTPS page directly, and the `web_search` tool to run web searches by fetching search-results pages (DuckDuckGo HTML endpoint, with Bing fallback) and returning ranked results with source URLs. When `GITHUB_TOKEN` is configured, PI may also search, list, fetch, create, and update GitHub resources and upload issue images, but only within `GITHUB_ALLOWED_REPOSITORY`. External content is labeled as untrusted and sanitized before it reaches the model; built-in coding tools remain disabled.
