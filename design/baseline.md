...
- `scheduled_prompts`: durable once/daily/weekly/monthly prompt schedules per conversation key, each attributed to its scheduling user, with fire bookkeeping (`last_run_at`, `completed` status); see [Scheduler tools](scheduler-tools.md) and [Scheduler execution engine](scheduler-execution.md).
...
- `schedule_prompt` stores once, daily, weekly, or monthly prompt schedules in UTC for the harness-injected conversation only after verifying (against live Discord state) that the harness-injected scheduling user is a member of that conversation, and `list_scheduled_prompts` and `cancel_scheduled_prompt` review and cancel those jobs without exposing other conversations' jobs; see [Scheduler tools](scheduler-tools.md).
