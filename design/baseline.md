...
- `scheduled_prompts`: durable once/daily/weekly/monthly prompt schedules per conversation key, each attributed to its scheduling user; see [Scheduler tools](scheduler-tools.md).
...
- `schedule_prompt` stores once, daily, weekly, or monthly prompt schedules in UTC for the harness-injected conversation only after verifying (against live Discord state) that the harness-injected scheduling user is a member of that conversation, and `list_scheduled_prompts` and `cancel_scheduled_prompt` review and cancel those jobs without exposing other conversations' jobs; firing a job re-applies the channel's allow-list permissions and re-checks membership before running it in that conversation's session; see [Scheduler tools](scheduler-tools.md).
