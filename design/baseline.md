...
- `scheduled_prompts`: durable once/daily/weekly/monthly prompt schedules per conversation key; see [Scheduler tools](scheduler-tools.md).
...
- `schedule_prompt` stores once, daily, weekly, or monthly prompt schedules in UTC for the harness-injected conversation only, and `list_scheduled_prompts` and `cancel_scheduled_prompt` review and cancel those jobs without exposing other conversations' jobs; see [Scheduler tools](scheduler-tools.md).
