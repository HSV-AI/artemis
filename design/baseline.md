...
- Due scheduled prompts run through the scheduler authorization gate into the full agent in the target conversation's serialized session; only JSON-conforming `message` responses post to Discord, `silent` responses post nothing, invalid replies trigger correction prompts (up to three tries) and are never posted, and jobs re-arm or complete with at-most-once delivery; on-demand runs through `run_scheduled_task` follow the identical path without the queue wait (the tool executes inside the live turn that holds it), consume the occurrence exactly like a fire, and are marked `trigger: on-demand` in the scheduler events; scheduler-fired generations never receive `run_scheduled_task`, so scheduled execution cannot recurse; see [Scheduler execution engine](scheduler-execution.md).
- PI or model-provider failures are logged without creating an assistant turn or sending a Discord response.
- Due scheduled prompts fire through the full agent in the target conversation's serialized session; `message` JSON responses post to Discord and persist the posted text, `silent` responses post nothing, invalid replies trigger correction prompts (up to three tries) and are never posted, and one-time jobs complete while recurring jobs re-arm across restarts; see [Scheduler execution engine](scheduler-execution.md).
