...
| Persona auto-selection | When an incoming Discord author's display name starts with the case-insensitive prefix `artemis`, apply the bundled `artemis` persona for that generation; otherwise keep the deployment-configured `PERSONA_PROFILE`. See [persona auto-selection](persona-auto-selection.md). |
