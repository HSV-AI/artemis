...
Detailed protocols and major features live in focused subdocuments so this baseline can remain a high-level description. The active governance contract is [Design documentation protocol](documentation-protocol.md). [Discord link-embed suppression](discord-link-embeds.md) enforces removal of link-preview cards on every outbound Discord message at the application layer, independent of the model, with a global default and per-channel override. [Persona profiles](persona-profile.md) let deployments vary identity and tone while preserving the common behavioral boundary. [Graph memory](memory.md) defines the Dgraph-backed fact lifecycle and PI tools. [Dgraph access control and namespaces](dgraph-access-control.md) defines authenticated service accounts and database-enforced separation between memory and public knowledge. [Model self-introspection](model-self-introspection.md) defines the `model_info` tool through which Artemis reports the provider and model it is actually running on from the live harness state. [HSVAI GraphRAG](hsvai-graphrag.md) defines synchronization, BM25 graph retrieval, and direct read-only DQL over public Huntsville AI transcripts and events. [HSVAI event catalog](hsvai-event-catalog.md) defines the reviewed seed and source validation. [Web search](web-search.md) defines the `web_search` tool that fetches search-results pages (DuckDuckGo HTML endpoint, with Bing fallback) and returns sanitized ranked results with source URLs, available to interactive and scheduled turns alike. [Native PI session persistence](pi-session-persistence.md) preserves PI tool, compaction, tree, and usage entries directly in SQLite across turns and restarts. [Channel timezone tools](timezone-tools.md) let each DM or guild channel store one validated IANA timezone and expose the current datetime, with the conversation identity always injected by the harness and every stored time UTC. [Scheduler tools](scheduler-tools.md) let the model schedule a prompt once, daily,
weekly, or monthly, or on a strict 5-field cron schedule, into its own conversation only — verified against live Discord membership at creation, attributed to the harness-injected scheduling user, re-checked against channel allow-lists and membership at fire time, persisted durably, reviewed with an optional completed/canceled audit history, cancelled on request with the record retained for audit, hard-removed by explicit pruning, recoverable after cancellation with a new schedule, edited in place while ongoing, and runnable immediately on demand through the same executor as a real fire — with every time stored as UTC. The [scheduler execution engine](scheduler-execution.md) claims due jobs atomically and runs each through the scheduler authorization gate into the full agent inside the target DM or Channel Group's durable session, validates the strict JSON response (`message` posts the content; `silent` posts nothing; an invalid reply triggers correction prompts inside the durable session, up to three tries, and is never posted), reconciles the claim so validated successes re-arm or complete jobs while denied, failed, or undeliverable runs stay claimable for a later tick, keeps the same executor available for immediate on-demand runs, and omits the on-demand run tool from scheduler-fired generations so scheduled execution cannot recurse. The complete catalog is maintained in the [design document index](README.md).
...
- Let each DM or guild channel set its own IANA timezone and let Artemis fetch the current datetime in that local time while storing all times as UTC.
- Let Artemis schedule a prompt to run once, daily, weekly, or monthly, or on an optional strict 5-field cron schedule (including day-of-month/day-of-week OR semantics), with jobs stored durably in UTC and restricted to the conversation the model is in.
- Let Artemis execute due scheduled prompts at fire time through the full agent in the target conversation's session, posting only validated JSON `message` responses and honoring `silent` without posting.
...
flowchart LR
    ...
    subgraph Compose[Docker Compose]
        ...
        PI --> WebSearch[web_search tool<br/>search-results pages<br/>ranked results with source URLs]
...
Only explicitly registered custom tools are enabled. `web_fetch` and allowlisted GitHub tools sanitize external reads; the `web_search` tool ([Web search](web-search.md)) performs web searches by fetching search-results pages and returning ranked results with source URLs; seven conversation-scoped [memory tools](memory.md) and the shared, read-only tools defined by [HSVAI GraphRAG](hsvai-graphrag.md) use Dgraph; the `model_info` tool reports the live registered provider and model from harness state, the [channel timezone tools](timezone-tools.md) manage each conversation's timezone, and the [scheduler tools](scheduler-tools.md) create, list (with optional audit history), cancel, prune, update (rewiring ongoing jobs in place and re-arming canceled ones), and immediately run scheduled prompts — bound to the harness-injected conversation key, attributed to the verified scheduling user (membership checked against live Discord state before anything is stored), pruning done only on explicit request with a hard delete, and storing all times as UTC. PI's built-in coding tools remain disabled. Novelty refusals are tool data; other tool failures follow the normal generation-failure path and produce no Discord response.
...
- `channel_timezones`: one stored IANA timezone per conversation key; see [Channel timezone tools](timezone-tools.md).
- `scheduled_prompts`: durable once/daily/weekly/monthly/cron prompt schedules per conversation key, each attributed to its scheduling user, with lifecycle bookkeeping (`last_run_at`, `completed` status, `claimed_until` for the engine's atomic claim-and-reconcile lifecycle, `next_run` for the engine-derived next-occurrence snapshot surfaced by the listing, prune hard-delete, update-driven re-arm of canceled records); see [Scheduler tools](scheduler-tools.md) and [Scheduler execution engine](scheduler-execution.md).
...
7. Start the scheduler execution engine, which claims due scheduled prompts atomically and fires them on a fixed interval once the Discord gateway completes its ready handshake — ticks before the handshake are deferred without listing or claiming — and runs them through the full agent; see [Scheduler execution engine](scheduler-execution.md).
...
The model schedules prompts through `schedule_prompt` (and manages them through the other scheduler tools): the schedule is one of the `once`/`daily`/`weekly`/`monthly` presets or, opt-in, a strict 5-field cron expression; both surfaces resolve in the schedule timezone (explicit, the channel's stored timezone, or UTC) and store all times as UTC; invalid or mutually exclusive schedule surfaces are refused without mutation. The execution engine polls the scheduled-prompt store on a fixed interval — deferring every poll until the Discord gateway's ready handshake completes, so no job fires into a client that cannot yet resolve its channels — and runs each due job in the conversation's serialized queue, so a fired turn can never race a live user turn; see [Scheduler execution engine](scheduler-execution.md) for the authoritative contract. The engine claims each due job atomically in storage before the gate runs (two pollers can never both run the same occurrence, and a claim left by a crashed run expires after ten minutes so the job is never permanently lost), submits the stored prompt framed with a strict JSON response contract to PI with full tool access, validates the agent's reply as one JSON object, corrects the agent inside the durable session with re-validation when the reply is invalid (at most three tries per fire) so broken JSON is never posted, posts `message` content to the target channel through the ordinary outbound path (embed suppression and Discord-safe splitting included), and honors `silent` by posting nothing. Only a validated success settles the claim: one-time jobs end `completed` after firing and cron and preset recurring jobs record `last_run_at` plus the engine-resolved next occurrence (`next_run`, the snapshot the listing surfaces) and re-arm, so a fired occurrence can never double-post and occurrences missed while Artemis was down collapse into a single late run — while denied, failed, invalid-response, or undeliverable runs release the claim and the job fires again on a later tick. An interactive turn can also run one of its conversation's scheduled prompts immediately via `run_scheduled_task`: the default is a reversible preview — a gated plain-turn run of the stored prompt that persists like any exchange, returns the response to the caller without posting, and leaves the occurrence pending (a one-time task stays active, a recurring schedule unchanged) — while `consume_next=true` goes through the same claim, gate, validation, and delivery as a real fire with the same reconcile; both are marked `trigger: on-demand` (previews also `mode: preview`) in the scheduler events, and the tool is omitted from scheduler-fired and preview generations so scheduled execution cannot recurse.
...
- No runtime code reads normalized messages to construct PI context; a fresh empty database bootstraps to the current schema (migrations 1 through 12), a verified migration-5 database receives incremental migrations 6 through 12 while preserving its history, and an existing database missing migration 5 fails startup with actionable guidance and no partial writes.
- `set_channel_timezone` stores a validated IANA identifier under the harness-injected conversation key only, rejects invalid or missing identifiers without writes, and `get_current_datetime` reports the UTC instant plus a DST-correct local rendering using the stored, explicit, or UTC fallback timezone.
- `schedule_prompt` stores once, daily, weekly, monthly, or strict-5-field-cron prompt schedules in UTC for the harness-injected conversation only after verifying (against live Discord state) that the harness-injected scheduling user is a member of that conversation; invalid or mutually exclusive schedule surfaces (cron plus preset fields, malformed cron, an unresolvable expression) are refused without mutation; `list_scheduled_prompts` reviews ongoing jobs (surfacing each record's persisted engine-derived next run, with recomputation fallback for legacy rows) and, with `include_history`, the conversation's completed and canceled audit records; `cancel_scheduled_prompt` stops jobs and flags them canceled while retaining the record; `prune_scheduled_prompt` hard-deletes records (single ID or bulk filters with a dry-run option); `update_scheduled_prompt` rewires an ongoing record's prompt text and/or schedule (preset or cron) in place, preserving its id, creation history, attribution, and fire marker, and re-arms a canceled record when given a new schedule (preserving its original prompt) while refusing completed records as retired history; `run_scheduled_task` immediately previews an ongoing record of this conversation (the default: a plain gated turn that consumes nothing and posts nothing, with the response returned for review) or — on the explicit `consume_next=true` opt-in — runs it through the same executor as a scheduled fire, consuming the occurrence, with the same response validation and delivery and clear refusals for unknown, foreign, canceled, and completed ids — none of which exposes or mutates other conversations' records; see [Scheduler tools](scheduler-tools.md).
- Due scheduled prompts are claimed atomically and then run through the scheduler authorization gate into the full agent in the target conversation's serialized session; only JSON-conforming `message` responses post to Discord, `silent` responses post nothing, invalid replies trigger correction prompts (up to three tries) and are never posted, validated successes settle the claim (re-arm or complete) while denied/failed/invalid/undeliverable runs release it for a later retry, and one-time jobs are never destroyed by a failed run; on-demand runs through `run_scheduled_task` follow the identical path without the queue wait (the tool executes inside the live turn that holds it), claim and reconcile the occurrence exactly like a fire, and are marked `trigger: on-demand` in the scheduler events — while the default call is gated a plain-preview turn that consumes nothing and posts nothing, returning the response for review — and scheduler-fired and preview generations never receive `run_scheduled_task`, so scheduled execution cannot recurse; see [Scheduler execution engine](scheduler-execution.md).
- PI or model-provider failures are logged without creating an assistant turn or sending a Discord response.
- Due scheduled prompts fire through the full agent in the target conversation's serialized session; `message` JSON responses post to Discord and persist the posted text, `silent` responses post nothing, invalid replies trigger correction prompts (up to three tries) and are never posted, and — claim-and-reconcile — validated successes settle one-time jobs as completed and re-arm recurring jobs across restarts while failed runs stay claimable for a later tick, so no crashed or denied run permanently destroys a job; see [Scheduler execution engine](scheduler-execution.md).
- Only `web_fetch`, `web_search`, token-gated GitHub tools, and scoped memory tools are enabled; `web_search` fetches search-results pages and returns sanitized ranked results with source URLs, `web_fetch` and GitHub tools sanitize external content, all populate the Available Tools prompt registry and include the Capability Gap Protocol, and none enable built-in coding tools.
