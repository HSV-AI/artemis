...
- Apply the schema bootstrap or rejection: a fresh empty database creates the current schema (migrations 1 through 10) in one transaction; a verified migration-5 database opens and applies incremental migrations 6, 7, 8, 9, and 10 without touching its history; an existing database missing migration 5 fails startup with an actionable error and no partial writes.
