...
flowchart LR
    ...
    Harness --> WebSearchTool[web_search tool]
...
Register `web_search` for every profile and conversation kind, scheduler-fired turns included, so scheduled prompts can search the web like any other registered tool. It accepts a required non-empty `query`, an optional integer `limit` between 1 and 20 (default 8), and an optional `engine` (`auto` default, `duckduckgo`, or `bing`). Reject blank queries and invalid limits before any network request.

The tool performs a web search by fetching a search-results page with the exact `web_fetch` conventions (direct HTTP GET, no model credentials, redirects followed, 100,000-character read bound). `auto` fetches `https://html.duckduckgo.com/html/?q=<encoded query>` first and falls back to `https://www.bing.com/search?q=<encoded query>` when the DuckDuckGo fetch fails or parses zero results; explicit `engine` values pin a single attempt.

Parse each page with the engine's own rules. DuckDuckGo: every `<a class="result__a">` anchor is one ranked result in document order and the nearest following `<a class="result__snippet">` anchor before the next title anchor is its snippet; unwrap `//duckduckgo.com/l/?uddg=<encoded source URL>` wrappers, dropping wrapped ad links (`ad_provider`/`ad_domain`), DuckDuckGo-internal links, and unwrappable links. Bing: each `<li class="b_algo">` organic block carries its title and wrapped link in the `<h2>` anchor and its snippet in the first `<p>`; unwrap `https://www.bing.com/ck/a?...&u=a1<base64url source URL>` wrappers, dropping `/aclk` ad links and unwrappable links. Strip markup, decode entities, deduplicate by source URL, and cap at the limit; accept only HTTP(S) targets during unwrapping.

Return a ranked, numbered list in which each entry carries the title, the unwrapped source URL, and a snippet; sanitize the rendered block with the same defenses as `web_fetch` (role-delimiter neutralization, instruction-override redaction, security notice) and label it as untrusted external web content naming the search URL. Expose the resolved engine, search URL, and structured results in the tool details. On non-successful engine responses raise `web_search (<engine>) failed (status <code>): <bounded text>`; in `auto` mode still try the other engine; when both engines are exhausted raise one aggregate error naming each engine's outcome. A pinned engine with zero parseable results (for example an anomaly/challenge page) errors `web_search found no results for "<query>" on <engine>`. See [Web search](web-search.md) for the authoritative contract.
...
An interactive turn can also run a stored job immediately, in one of two modes selected by `consume_next` (default `false`): `run_scheduled_task` resolves the id among its own conversation's history, refuses blank, unknown, foreign, canceled, and completed records, and — for an `active` record — delegates to the engine's executor for the requested mode (both wired into the gateway as one lazily resolved `ScheduledTaskRunner`). The default **preview** (`SchedulerRunner.runScheduledTaskPreview`) takes no claim and consumes nothing: it runs the same fire-time authorization gate through the preview variant `ConversationService.runScheduledPromptPreviewInline` — the identical scope and membership checks without entering the per-conversation queue, because the tool already executes inside the live turn that holds the queue slot — and then performs one plain generation of the stored prompt verbatim in the durable session, flagged `scheduledRun`, persisted like any exchange, with no scheduler-wide JSON framing, no correction retries, and no delivery of anything to a channel; the response text returns to the tool as a `previewed` result while the occurrence stays pending (one-time jobs stay active, recurring jobs keep their next occurrence). The explicit **fire** (`consume_next=true`, `SchedulerRunner.runScheduledTaskNow`) consumes the occurrence with the identical path (one-time jobs complete; recurring jobs re-arm via `last_run_at`), runs the same fire-time authorization gate through the inline variant `ConversationService.runScheduledPromptInline`, and validates and delivers the response through the shared engine path, reconciling the claim identically (settled on posted/silent success, released on denied/failed/invalid/undelivered runs so the task remains scheduled) and reporting the outcome back to the tool. Scheduler-fired generations are flagged `scheduledRun` on `PiGenerationInput` (preview generations included, set by the conversation service) and must not register `run_scheduled_task` (with their own cached prompt registry), so a fired task can never trigger further runs and on-demand execution cannot recurse. Scheduler events carry a `trigger` field distinguishing `scheduled` engine fires from `on-demand` tool runs, and preview runs are additionally marked `mode: preview`.
...
- `web_search` validates query and limit before fetching, fetches search-results pages with the `web_fetch` conventions (DuckDuckGo first, Bing fallback in `auto` mode, pins honored), returns sanitized ranked results with unwrapped source URLs, and reports clear per-engine and aggregate failures.
